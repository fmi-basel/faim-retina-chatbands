...
def construct_model(model_name, n_classes=2, n_channels=1, final_weights=None):
    ...
    def _construct():
        ...
        if model_name == 'unet':
            backbone = GenericUnetBase(input_shape=input_shape,
        ...
        intermediate = keras.layers.Conv2D(n_features,
                                           kernel_size=3,
                                           activation='relu',
        ...
        prediction = keras.layers.Conv2D(1,
                                         kernel_size=3,
                                         activation='sigmoid',
                                         padding='same')(intermediate)
        ...
        if downsampling_factor > 1:
            prediction = keras.layers.Conv2DTranspose(
                ...
                padding='same')(prediction)
            prediction = DynamicTrimmingLayer(name='outer_dtrim')(
                [input, prediction])
    ...
    #model = _construct()
    model = load_model(
            os.path.join('models/N500-percentile_max-block-20_231027/segm/unet-L4-D1-None/run002/', 'model_architecture.yaml'),
            os.path.join('models/N500-percentile_max-block-20_231027/segm/unet-L4-D1-None/run002/', 'model_best.h5')) 
...
def get_outdir(outdir, dataset_folder, model_name):
    ...
    def _propose():
        '''propose and create an output folder for each new run.
        '''
        count = 0
        while True:
            outdir = os.path.join(
...
def main(*args):
    ...
    dataset = get_dataset(dataset_folder=ARGS.dataset_folder,
    ...
    callbacks = [
        ...
        keras.callbacks.ModelCheckpoint(os.path.join(outdir, 'model_best.h5'),
                                        monitor='val_loss',
                                        verbose=0,
                                        save_best_only=True,
                                        save_weights_only=True,
                                        period=1),
        keras.callbacks.ModelCheckpoint(os.path.join(outdir,
                                                     'model_latest.h5'),
                                        verbose=0,
                                        save_best_only=False,
                                        save_weights_only=True,
                                        period=1),
        ExtendedTensorBoard(log_dir=os.path.join(outdir, 'logs'),
                            write_graph=True,
                            write_grads=False,
                            write_images=False),
    ]
    ...
    model.fit(dataset['training_iter']['image'],
